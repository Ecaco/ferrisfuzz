...
/// This will be your CURRENT, working jaro_winkler_opt body: chars() + Vec<bool> match
...
mod tests {
    ...
    fn anchor_textbook_values() {
        ...
        for (a, b, want) in cases {
            let got = jaro_winkler_opt(a, b, None, None, None, None).unwrap();
    ...
    fn fuzz_band(iters: usize, min_len: usize, span: u64, seed0: u64) {
        ...
        for _ in 0..iters {
            ...
            let got = jaro_winkler_opt(sa, sb, None, None, None, None).unwrap();
    ...
    #[test]
    fn edge_cases() {
        assert_eq!(jaro_winkler_opt("", "", None, None, None, None).unwrap(), 1.0);
        assert_eq!(jaro_winkler_opt("abc", "", None, None, None, None).unwrap(), 0.0);
        assert_eq!(jaro_winkler_opt("", "abc", None, None, None, None).unwrap(), 0.0);
        assert_eq!(jaro_winkler_opt("hello", "hello", None, None, None, None).unwrap(), 1.0);
        assert_eq!(jaro_winkler_opt("abc", "xyz", None, None, None, None).unwrap(), 0.0);
        assert_eq!(jaro_winkler_opt("a", "a", None, None, None, None).unwrap(), 1.0);
        assert_eq!(jaro_winkler_opt("a", "b", None, None, None, None).unwrap(), 0.0);
    }
    ...
    fn non_ascii_routes_correctly() {
        ...
        assert_eq!(jaro_winkler_opt("café", "café", None, None, None, None).unwrap(), 1.0);
    }
    ...
    fn case_insensitive_folds() {
        assert_eq!(jaro_winkler_opt("MARTHA", "martha", None, None, Some(true), None).unwrap(), 1.0);
        let cs = jaro_winkler_opt("MARTHA", "martha", None, None, None, None).unwrap();
        assert!(cs < 1.0, "case-sensitive should not treat MARTHA==martha");
    }

    #[test]
    fn prefix_weight_bounds() {
        assert!(jaro_winkler_opt("a", "b", Some(0.5),  None, None, None).is_err());
        assert!(jaro_winkler_opt("a", "b", Some(-0.1), None, None, None).is_err());
        assert!(jaro_winkler_opt("martha", "marhta", Some(0.2), None, None, None).is_ok());
        let low  = jaro_winkler_opt("marhta", "martha", Some(0.05), None, None, None).unwrap();
        let high = jaro_winkler_opt("marhta", "martha", Some(0.20), None, None, None).unwrap();
    ...
    fn max_len_rejects() {
        ...
        assert!(jaro_winkler_opt(&long, "a", Some(10.0), None, None, None).is_err());
    }
    ...
    fn score_cutoff_is_a_floor() {
        ...
        assert_eq!(jaro_winkler_opt("martha", "marhta", None, None, None, Some(0.99)).unwrap(), 0.0);
        // …while a floor of 0.90 lets it through unchanged.
        let v = jaro_winkler_opt("martha", "marhta", None, None, None, Some(0.90)).unwrap();
        assert!(v > 0.90);
    }
}